...
updatePreviewFpsParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    String8 s8FrameRateList("10,20,15,24,30,60,120");
    ...
    mParameters.set(CameraParameters::KEY_SUPPORTED_PREVIEW_FPS_RANGE, "(15000,15000),(24000,24000),(30000,30000),(60000,60000),(120000,120000)");
    //
    return  true;
}
...
updateExposureCompensationParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    if(i4CamMode < 0) {     //Google camera
        MY_LOGD_IF(1, "%s=%s;%s=%s;%s=%s",
...
updatePreferredPreviewSize()
{
    ...
    unsigned int maxResolutionUp = 0xFFFFFF;
    unsigned int maxResolution = 0;
    unsigned int resolutionTmp = 0;
    ...
    if ( ! vdoSizes.isEmpty() )
    {
        // from largest video support video resultion to search matched support preview resolution
        for ( unsigned int i = 0 ; i < vdoSizes.size() ; i++ )
        {
            maxWidth = 0;
            maxHeight = 0;
            for ( Vector<Size>::iterator vdoIt = vdoSizes.end()-1; vdoIt != vdoSizes.begin()-1; vdoIt-- )
            {
                resolutionTmp = vdoIt->width * vdoIt->height; 
                // if current resultion is same as the upper limit, check the width                    
                if ( resolutionTmp == maxResolutionUp)
                {
                    if( vdoIt->width >= lastMaxWidth )
                    {
                        continue;
                    }
                    maxResolution = resolutionTmp;
                    maxWidth = vdoIt->width;
                    lastMaxWidth = vdoIt->width;
                    maxHeight = vdoIt->height;
                }
                // if curret resolution is in between the current max and upper limit, select current resolution
                else if ( resolutionTmp > maxResolution && resolutionTmp < maxResolutionUp )
                {
                    maxResolution = resolutionTmp;
                    maxWidth = vdoIt->width;
                    lastMaxWidth = vdoIt->width;
                ...
                // if current resolution is same as current max, check the width
                else if ( resolutionTmp== maxResolution)
                {
                    if ( vdoIt->width <= maxWidth )
                    {
                        continue;
                    }
                    maxResolution = resolutionTmp;
            ...
            maxResolutionUp = maxResolution;
            maxResolution = 0;
            for ( Vector<Size>::iterator prvIt = prvSizes.end()-1; prvIt != prvSizes.begin()-1; prvIt-- )
            {
                if ( maxWidth == prvIt->width && maxHeight == prvIt->height )
                {
                    snprintf(psize, sizeof(psize),"%dx%d", prvIt->width, prvIt->height);
...
updateDefaultParams1_ByDefault()
{
    ...
    mParameters.set(MtkCameraParameters::KEY_FEATURE_MAX_FPS,"30@VFB+EIS");
    // disable facedetection!!!
    mParameters.set(CameraParameters::KEY_MAX_NUM_DETECTED_FACES_HW, 15);
    
    mParameters.set(MtkCameraParameters::KEY_DYNAMIC_FRAME_RATE,"true");
    mParameters.set(MtkCameraParameters::KEY_DYNAMIC_FRAME_RATE_SUPPORTED,"true");
    
    mParameters.set(MtkCameraParameters::KEY_ENG_VIDEO_RAW_DUMP_RESIZE_TO_2M_SUPPORTED,"true");
    mParameters.set(MtkCameraParameters::KEY_ENG_VIDEO_RAW_DUMP_RESIZE_TO_4K2K_SUPPORTED,"true");
    mParameters.set(MtkCameraParameters::KEY_ENG_VIDEO_RAW_DUMP_CROP_CENTER_2M_SUPPORTED,"true");
    ...
    mParameters.set(MtkCameraParameters::KEY_SENSOR_TYPE,252);
    mParameters.set(MtkCameraParameters::KEY_ENG_SENOSR_MODE_SUPPORTED,0);
    ...
    mParameters.set(MtkCameraParameters::KEY_ENG_SENSOR_SHADNING_SUPPORTED,"true");
    mParameters.set(MtkCameraParameters::KEY_ENG_SENSOR_AWB_SUPPORTED,"true");
    ...
    // two sensor mode
    mParameters.set(MtkCameraParameters::KEY_ENG_SENOSR_MODE_SLIM_VIDEO1_SUPPORTED,1);
    mParameters.set(MtkCameraParameters::KEY_ENG_SENOSR_MODE_SLIM_VIDEO2_SUPPORTED,1);
    //
    ...
    mParameters.set(MtkCameraParameters::KEY_CSHOT_INDICATOR,"true");
    mParameters.set("cshot-indicator-supported",true);
    
    mParameters.set(CameraParameters::KEY_MAX_NUM_FOCUS_AREAS, 1);
    mParameters.set(CameraParameters::KEY_MAX_NUM_METERING_AREAS, 9);
    ...
    mParameters.set(CameraParameters::KEY_SUPPORTED_FLASH_MODES,"off,on,auto,red-eye,torch");

	mParameters.set(CameraParameters::KEY_FOCUS_MODE,"auto");
	mParameters.set(CameraParameters::KEY_SUPPORTED_FOCUS_MODES,"auto,macro,infinity,continuous-picture,continuous-video,manual,fullscan");
    ...
    mParameters.set(CameraParameters::KEY_SUPPORTED_JPEG_THUMBNAIL_SIZES, "0x0,160x128,256x192");
    ...
    mParameters.set(CameraParameters::KEY_HORIZONTAL_VIEW_ANGLE, 50);
    mParameters.set(CameraParameters::KEY_VERTICAL_VIEW_ANGLE, 50);
...
//    mParameters.set(MtkCameraParameters::KEY_ISP_MODE, 0);     //0: normal
    ...
    //mParameters.set(MtkCameraParameters::KEY_VR_BUFFER_COUNT, 10);
    ...
    mParameters.set(MtkCameraParameters::KEY_FB_EXTREME_BEAUTY, CameraParameters::FALSE);
    mParameters.set("fb-extreme-beauty-supported","true");
    ...
    //  Android parameters
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    //mParameters.set(CameraParameters::KEY_PREVIEW_SIZE, "1280x720");
    
    //mParameters.set(CameraParameters::KEY_PREVIEW_SIZE, "640x480");
    
    mParameters.set(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES, "176x144,320x240,352x288,480x320,480x368,640x480,720x480,800x480,800x600,864x480,960x540,1280x720,1440x1080,1920x1080,1920x1088,1920x1440,2560x1440,2560x1920");
    //
    mParameters.set(CameraParameters::KEY_PREVIEW_FRAME_RATE, "30");
    mParameters.set(CameraParameters::KEY_SUPPORTED_PREVIEW_FRAME_RATES, "10,20,15,24,30,60,120");
    ...
    // hdr mode NOT working in 12288x6912,10624x7968
    mParameters.set(CameraParameters::KEY_SUPPORTED_PICTURE_SIZES, "320x240,640x480,1024x768,1280x720,1280x768,1280x960,1600x912,1600x1200,2048x1536,2048x1152,2560x1440,2560x1920,2880x1728,3264x2448,3328x1872,3600x2160,3672x2754,3584x2016,4608x2592,4864x2736,4096x3072,4160x3120,5120x2880,5120x3840,6144x3456,5312x3984,7952x4480,7536x5648");
    //
    //mParameters.set(CameraParameters::KEY_VIDEO_SIZE, "1280x720");
    mParameters.set(CameraParameters::KEY_SUPPORTED_VIDEO_SIZES, "176x144,320x240,352x288,480x320,640x480,720x480,864x480,1280x720,1920x1080,1920x1088,3840x2160,3840x2176");
    //
    //mParameters.set(CameraParameters::KEY_WHITE_BALANCE, CameraParameters::WHITE_BALANCE_AUTO);
    mParameters.set(CameraParameters::KEY_SUPPORTED_WHITE_BALANCE, "auto,incandescent,fluorescent,warm-fluorescent,daylight,cloudy-daylight,twilight,shade");
    //
    //mParameters.set(CameraParameters::KEY_EFFECT, CameraParameters::EFFECT_NONE);
    mParameters.set(CameraParameters::KEY_SUPPORTED_EFFECTS, "none,mono,negative,sepia,aqua,whiteboard,blackboard,posterize,nashville,hefe,valencia,xproll,lofi,sierra,walden");
    //
    //mParameters.set(CameraParameters::KEY_ANTIBANDING, CameraParameters::ANTIBANDING_OFF);
    mParameters.set(CameraParameters::KEY_SUPPORTED_ANTIBANDING, "off,50hz,60hz,auto");
    //
    //mParameters.set(CameraParameters::KEY_SCENE_MODE, CameraParameters::SCENE_MODE_AUTO);
    mParameters.set(CameraParameters::KEY_SUPPORTED_SCENE_MODES, "auto,portrait,landscape,night,night-portrait,theatre,beach,snow,sunset,steadyphoto,fireworks,sports,party,candlelight,hdr");
    //
    //
    //
    //mParameters.set(MtkCameraParameters::KEY_VIDEO_HDR,"off");
    mParameters.set("video-hdr-values","off,on");
    //
    //mParameters.set(CameraParameters::KEY_VIDEO_STABILIZATION, CameraParameters::FALSE);
    ...
    mParameters.set(CameraParameters::KEY_MAX_EXPOSURE_COMPENSATION, 3);
    mParameters.set(CameraParameters::KEY_MIN_EXPOSURE_COMPENSATION, -3);
    mParameters.set(CameraParameters::KEY_EXPOSURE_COMPENSATION_STEP, "1.0");
    //
    mParameters.set(CameraParameters::KEY_VIDEO_SNAPSHOT_SUPPORTED, CameraParameters::TRUE);
    ...
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_AF_LAMP_MODE), "off,on,auto");
    //  iso-speed 
    //mParameters.set(MtkCameraParameters::KEY_ISO_SPEED, MtkCameraParameters::ISO_SPEED_AUTO);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_ISO_SPEED), "auto,100,200,400,800,1600");
    //  ZSD-mode 
    //mParameters.set(MtkCameraParameters::KEY_ZSD_MODE, MtkCameraParameters::OFF);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_ZSD_MODE), "off,on");
    //  Capture Mode
    //mParameters.set(MtkCameraParameters::KEY_CAPTURE_MODE, MtkCameraParameters::CAPTURE_MODE_NORMAL);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_CAPTURE_MODE), "normal,face_beauty,continuousshot,smileshot,bestshot,autorama,mav,asd,motiontrack");
    //  Hue
    //mParameters.set(MtkCameraParameters::KEY_HUE, MtkCameraParameters::MIDDLE);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_HUE), "low,middle,high");
    //  Edge
    //mParameters.set(MtkCameraParameters::KEY_EDGE, MtkCameraParameters::MIDDLE);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_EDGE), "low,middle,high");
    //  Saturation
    //mParameters.set(MtkCameraParameters::KEY_SATURATION, MtkCameraParameters::MIDDLE);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_SATURATION), "low,middle,high" );
    //  Brightness
    //mParameters.set(MtkCameraParameters::KEY_BRIGHTNESS, MtkCameraParameters::MIDDLE);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_BRIGHTNESS), "low,middle,high");
    //  Constrast
    //mParameters.set(MtkCameraParameters::KEY_CONTRAST, MtkCameraParameters::MIDDLE);
    mParameters.set(getValuesKeyName(MtkCameraParameters::KEY_CONTRAST), "low,middle,high");
    //
	mParameters.set("mfb-values","off,mfll,ais");
    //mParameters.set("mfb","off");
...
bool
ParamsManager::
evaluateHalAppMode(String8& mode) const
{
    RWLock::AutoRLock _lock(mRWLock);
...
*
*******************************************************************************/
#define PROPERTY_KEY_HAL_FORCEAPPMODE    "hal.forceappmode"
bool
ParamsManager::
setForceHalAppMode(String8 const& mode)
{
    NSCam::Utils::Property::set( String8(PROPERTY_KEY_HAL_FORCEAPPMODE), mode);
    return true;
}
...
bool
ParamsManager::
evaluateHalAppModeUtility(String8& mode) const
{
    int32_t const i4CamMode = 0 ; // getInt(MtkCameraParameters::KEY_CAMERA_MODE);
    MY_LOGD("+ KEY_CAMERA_MODE:%d", i4CamMode);
    //
    //  evaluate priority:
    //      1. Client App Mode
    //      2. Force Hal App mode
    //      3. if recording hint is set, use default
    //      4. if camera mode = normal, zsd-mode is "on" (zsd)
    //      5. otherwise, use camera mode
    //
    //  If Client App Mode != <Default> App, then.
    //
    if  ( ms8ClientAppMode != MtkCameraParameters::APP_MODE_NAME_DEFAULT )
    {
        mode = ms8ClientAppMode;
        goto lbExit;
    }
    //  Force Hal App mode
    if  ( NSCam::Utils::Property::tryGet(String8(PROPERTY_KEY_HAL_FORCEAPPMODE), mode ) )
    {
        if( mode != "" )
        {
            MY_LOGD("force mode: %s", mode.string());
            //clear property
            NSCam::Utils::Property::set( String8(PROPERTY_KEY_HAL_FORCEAPPMODE), String8());
            goto lbExit;
        }
    }
    // check recording-hint
    {
        ...
        {
            recordingHint =  ( ::strcmp(p, CameraParameters::TRUE) != 0 ) ? false : true;
        }
        //
        if( recordingHint )
        {
            MY_LOGD("recording hint = true, use default-mode");
            mode = MtkCameraParameters::APP_MODE_NAME_DEFAULT;
            goto lbExit;
        }
    }
    //
    ...
    if  ( i4CamMode == MtkCameraParameters::CAMERA_MODE_NORMAL )
    {
        ...
        {
            // if zsd is on, use zsd mode
            MY_LOGD("zsd is \"on\": default zsd-mode");
    ...
    if  ( i4CamMode == MtkCameraParameters::CAMERA_MODE_MTK_PRV )
    {
        ...
        if  ( ! pszZsdMode || 0 != ::strcmp(pszZsdMode, MtkCameraParameters::ON) )
        {
            mode = MtkCameraParameters::APP_MODE_NAME_MTK_PHOTO;
        }
        else
        {
            mode = MtkCameraParameters::APP_MODE_NAME_MTK_ZSD;
    ...
    {
        mode = MtkCameraParameters::APP_MODE_NAME_MTK_VT;
        goto lbExit;
    }
    //
    if  ( i4CamMode == MtkCameraParameters::CAMERA_MODE_MTK_VDO )
    {
        mode = MtkCameraParameters::APP_MODE_NAME_MTK_VIDEO;
        goto lbExit;
    }
    //
    //
...
updateHalAppMode()
{
    ...
    if( !evaluateHalAppModeUtility(s8HalAppMode) )
    {
        MY_LOGE("evaluateHalAppMode fails");
        return false;
    }
    //
lbExit:
