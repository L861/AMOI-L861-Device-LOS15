...
{
    MY_LOGD("%d: + AutoWLock", getOpenId());
    MyRWLock::AutoWLock _lock(mRWLock);
    //
...
getMultFrameBlending() const
{
    ...
    if  (0 == ::strcmp(pMfb, MtkCameraParameters::KEY_MFB_MODE_AIS) )
    {
        ...
        {
            MY_LOGD(" Discard AIS due to manual ISO");
            mfbMode = 0;
            return mfbMode;
        }
    }
...
getIfFirstPreviewFrameAsBlack()
{
    ...
    MY_LOGD("firsFrameBlack(%d)",firsFrameBlack);
    if (firsFrameBlack == 1)
    {
        ret = true;
    }
...
{
    MY_LOGD("AutoWLock %s=%s", key, value);
    MyRWLock::AutoWLock _lock(mRWLock);
...
{
    MY_LOGD("AutoWLock %s=%d", key, value);
    MyRWLock::AutoWLock _lock(mRWLock);
    mParameters.set(key, value);
}
...
AddPreviewSizeForPanel()
{
    ...
    if(p)
    {
        ...
        MY_LOGD("DisplayRotationSupported ret = %d (KEY_CAMERA_MODE=%d)(ms8ClientAppMode=%s)", bDisplayRotationSupported ? 1 : 0, CAM_MODE, ms8ClientAppMode.string());
    }
    //
