...
updateUserTypeParams(String8 const& s8KeyName, NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    //
    if  ( s8KeyName == MtkCameraParameters::KEY_DNG_SUPPORTED )
    {
        return  updateDNGParams(rFeatureInfo);
    }
    //
    return  false;
}
...
updatePreviewFpsParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    String8 s8FrameRateList("10,20");
    String8 s8UpdateFpsRangeList;
    ...
    else
    {
        for (List< Vector<int> >::iterator it = itemList.begin(); it != itemList.end(); it++)
        {
            ...
            if((*it)[1]/1000 <= 30)
            {
                if(!s8UpdateFpsRangeList.isEmpty())
                {
                    s8UpdateFpsRangeList+= String8::format(",(%d,%d)",(*it)[0],(*it)[1]);
                }
                else
                {
                    s8UpdateFpsRangeList+= String8::format("(%d,%d)",(*it)[0],(*it)[1]);
    ...
    //
    mDefaultPreviewFps = getInt(CameraParameters::KEY_PREVIEW_FRAME_RATE);
...
bool
ParamsManager::
updateDNGParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    String8 const& rs8Supported = rFeatureInfo.getDefaultItem();
    //
    MY_LOGD_IF(1, "%s=%s", MtkCameraParameters::KEY_DNG_SUPPORTED, rs8Supported.string());
    //
    mParameters.set(MtkCameraParameters::KEY_DNG_SUPPORTED, rs8Supported.string());
    MY_LOGD("-");
    //
    return  true;
}
...
updateDefaultParams1()
{
    ...
    //
    //  For test.
    mParameters.set("preview-size-values", "176x144,320x240,352x288,480x320,480x368,640x480,720x480,864x480,960x540,1280x720");
    mParameters.set("preview-size-values", "320x240,352x288,704x576,640x480,720x480,960x720,960x540,1280x720");
    
    mParameters.set("preview-frame-rate-values","10,20,15,24,30,60,120");
    mParameters.set("preview-frame-rate","30");
    
    mParameters.set("preview-fps-range","5000,60000");
    mParameters.set("preview-fps-range-values","(5000,60000)");

    
    mParameters.set(CameraParameters::KEY_VIDEO_FRAME_FORMAT, CameraParameters::PIXEL_FORMAT_YUV420SP);
    
    //  Panel Size
    if(getDisplayRotationSupported())
    {
        MY_LOGD("Reset Panel Size to 0x0");
        mParameters.set(MtkCameraParameters::KEY_PANEL_SIZE, "0x0");
    }
    //
    MY_LOGD("-");
    return  true;
...
//    mParameters.set(MtkCameraParameters::KEY_ISP_MODE, 0);     //0: normal
    ...
    mParameters.set(MtkCameraParameters::KEY_FB_SMOOTH_LEVEL_Default, 3);
    ...
    mParameters.set(MtkCameraParameters::KEY_FB_SKIN_COLOR_Default, 9);
    ...
    // first preview frame is black or not
    mParameters.set(MtkCameraParameters::KEY_FIRST_PREVIEW_FRAME_BLACK, 0);
    //----------------------------------------------------------------------------------------------
    //  MTK proprietary parameters
    //----------------------------------------------------------------------------------------------
    MY_LOGD("-");
...
{
    int32_t const i4CamMode = getInt(MtkCameraParameters::KEY_CAMERA_MODE);
    char halForceMode[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("hal.appmode", halForceMode, "None");
    MY_LOGD("+ KEY_CAMERA_MODE:%d", i4CamMode);
    MY_LOGD("HAL Force Mode = [%s]", halForceMode);
    //  Force Hal App mode (system property)
    if(strcmp(halForceMode, "None")) {
        MY_LOGD("Applying Halforcemode via property");
        mode = String8(halForceMode);
        goto lbExit;
    }
    ...
    if  ( i4CamMode == MtkCameraParameters::CAMERA_MODE_MTK_PRV )
    {
        ...
        {
            int32_t i4ZsdMode = 0;
            i4ZsdMode = property_get_int32("camera.zsdmode", 0);
...
updateHalAppMode()
{
    ...
    //
    if( !evaluateHalAppMode(s8HalAppMode) )
    {
