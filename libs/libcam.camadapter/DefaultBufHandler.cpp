...
// #pramga GCC diagnostic ignored "-Wno-parentheses"
    ...
    if(isDequeProvider)
    {
        ...
        if( doCacheInvalid )
        {
            if( !tempBuffer->syncCache(eCACHECTRL_INVALID) )
                MY_LOGE("invalid cache failed imgbuf");
        }
...
enqueBuffer(MUINT32 const data, IImageBuffer const * pImageBuffer)
{
    ...
    if(bufQueIdx >= 0)
    {
        ...
        {
            MY_LOGE("mvBufQueNode[%d] size(%ul) = 0",
                    bufQueIdx,
                    (unsigned int) mvBufQueNode[bufQueIdx].size());
            return MFALSE;
        }
    }
    else
        ...
        else
        {
            ...
            */
            ...
            if(isAPClientFromFD)
            {
                ...
                sp<IImgBufProvider> pBufProvider = NULL;
                ...
                {
                    MY_LOGD("APClient size:%zu, fdClient size:%zu", APClientnode.getImgBuf()->getBufSize() ,  keepImgBufQueNode.getImgBuf()->getBufSize());
    ...
    else
    {
        MY_LOGE("Addr(0x%X != 0x%X),Port(%d), drop it!",
                (unsigned int) keepAddr,
                (unsigned int) enqueAddr,
                mvBufQueNode[bufQueIdx][0].getCookieDE());
        return MFALSE;
    }
}
