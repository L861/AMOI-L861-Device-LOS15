...
bool Utils::exec_tp(const string path) {
	string str = "/system/vendor/bin/thermal_manager /system/vendor/etc/.tp/" + path + " &";
	ALOGV("%s: setTProfile: %s", __func__, str.c_str()); 
...
bool Utils::screenIsOn() {
	int brightness = 0;
	if (!Utils::read("/sys/class/leds/lcd-backlight/brightness", brightness)) {
		return true;
	}
	return (brightness > 0);
}
