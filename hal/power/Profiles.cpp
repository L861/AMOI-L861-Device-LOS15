...
#define XML_GET_CPUCLUSTER()                                                                          \
	PROFILE_SET     (cpu, governor,     "cpu/governor",     "interactive");                             \
	PROFILE_SET_UINT(cpu, num_limit_power_serv,     "cpu/num_limit_power_serv",     "8");               \
	PROFILE_SET_UINT(cpu, freq_min,     "cpu/freq_min",     "403000");                            \
	PROFILE_SET_UINT(cpu, freq_max,     "cpu/freq_max",     "1950000"); \
	PROFILE_SET_UINT(cpu, freq_hispeed, "cpu/freq_hispeed", "1950000"); \
	PROFILE_SET_UINT(cpu, freq_boost,   "cpu/freq_boost",   "0");                                 \
	if (profile->cpu.cores.enabled) {                                                                      \
		PROFILE_SET_BOOL(cpu.cores, core1, "cpu/cores/core1", "true");                            \
		PROFILE_SET_BOOL(cpu.cores, core2, "cpu/cores/core2", "true");                            \
		PROFILE_SET_BOOL(cpu.cores, core3, "cpu/cores/core3", "true");                            \
		PROFILE_SET_BOOL(cpu.cores, core4, "cpu/cores/core4", "true");                            \
		PROFILE_SET_BOOL(cpu.cores, core5, "cpu/cores/core5", "true");                            \
		PROFILE_SET_BOOL(cpu.cores, core6, "cpu/cores/core6", "true");                            \
		PROFILE_SET_BOOL(cpu.cores, core7, "cpu/cores/core7", "true");                            \
		PROFILE_SET_BOOL(cpu.cores, core8, "cpu/cores/core8", "true");                            \
	}

#define XML_GET_CPUGOV()                                            \
	{                                                                   \
		char objPath[255];                                              \
		strcpy(objPath, path);                                          \
		strcat(objPath, "/cpu/governor_data");                \
		xmlXPathObject *obj =                                           \
			xmlXPathEvalExpression ((xmlChar *)objPath, ctx);           \
		if (!obj->nodesetval->nodeTab ||                                \
			!obj->nodesetval->nodeTab[0] ||                             \
			...
			do {                                                        \
				if (node->children) {                                   \
					strcpy(profile->cpu.governor_data[index].name, \
						(const char *)node->name);                      \
					strcpy(profile->cpu.governor_data[index].data, \
						(const char *)node->children->content);         \
					index++;                                            \
				}                                                       \
				node = node->next;                                      \
			} while (node);                                             \
			profile->cpu.governor_data[index].name[0] = 0;         \
			profile->cpu.governor_data[index].data[0] = 0;         \
		}                                                               \
	}
...
void Profiles::loadProfileImpl(SecPowerProfile *profile, xmlXPathContext *ctx, const char *path) {
	...
	PROFILE_INIT2 (cpu,	  cores,   				"cpu/cores");
	...
	PROFILE_INIT (thermal_profile,             "thermal_profile");
	...
	if (profile->thermal_profile.enabled) {
		PROFILE_SET(thermal_profile, profilename, "thermal_profile/profilename", "thermal.conf");
	}
