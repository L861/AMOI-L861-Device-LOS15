...
int pseudo_device_read(void *vargp)
{
    ...
    // avoid vt service connect to vt pty port before mux logic channel created
    // skip pty read function when mux channel is not created
    if (mux_setup_chnl_complete == 0) {
        LOGMUX(LOG_ERR, "access channel:%s too early, opened:%d",channel->devicename, channel->opened);
        LOGMUX(LOG_ERR, "avoid access channel before Finish MUX Channel Setup Procedure");
	...
	if (!channel->opened && !LackofAtciChannel) {
         ...
         *  channel->tmp = calloc(1, len + channel->remaining);
...
{
    GSM0710_Buffer *buf = (GSM0710_Buffer *)calloc(1, sizeof(GSM0710_Buffer));
    ...
    if (buf != NULL) {
        ...
        pthread_cond_init(&buf->bufferready_signal, NULL);
        pthread_cond_init(&buf->newdataready_signal, NULL);
    }
    return buf;
}
...
l_begin:
    ...
    while (gsm0710_buffer_length(buf) > 0) {
        if (!buf->adv_found_esc && GSM0710_FRAME_ADV_FLAG == *(buf->readp)) { /* Whole frame parsed for escape chars, closing flag found */
            ...
            if ((frame = (GSM0710_Frame *)calloc(1, sizeof(GSM0710_Frame))) != NULL) { /* frame is sane, allocate memory for it */
            ...
            if (frame->length > 0) {
                if ((frame->data = (unsigned char *)calloc(1, sizeof(char) * frame->length))) {
    ...
    if (frame->length > 0) {
        ...
        if ((type & GSM0710_CR) == GSM0710_CR) {
            ...
            default:
                ...
                if ((response = calloc(1, sizeof(char) * (2 + type_length))) != NULL) {
                    i = 0;
...
void signal_treatment(int param)
{
    ...
    default:
        ...
        if(stop_muxd != 1){
            stop_muxd = 1;
            LOGMUX(LOG_ERR, "Close CCCI port,fd=%d",serial.fd);
            SYSCHECK1(shutdown_devices(SHUTDOWN_DEV_W_ACTIVE_FINALIZED));
        }
        exit(0);
        break;
    }
...
int watchdog(Serial *serial)
{
    ...
    case MUX_STATE_INITILIZING:
        ...
        Poll_Thread_Arg *poll_thread_arg = (Poll_Thread_Arg *)calloc(1, sizeof(Poll_Thread_Arg)); //iniitialize pointer to thread args ;
        ...
        /* Create thread for polling on serial device (mux input) and writing to GSM0710 mux buffer */
        Poll_Thread_Arg *ut_poll_thread_arg = (Poll_Thread_Arg *)calloc(1, sizeof(Poll_Thread_Arg)); //iniitialize pointer to thread args ;
    ...
    case MUX_STATE_MUXING:
    {
        ...
        if (pts_reopen == 1) {
            for (i = 1; i < GSM0710_MAX_CHANNELS; i++) {
                if (channellist[i].reopen == 1) {
                    if (GSM0710MUXD_SUCCESS != setup_pty_interface(&channellist[i])) {
                        //Exit main function if channel couldn't be allocated
                        err = GSM0710MUXD_RESTART_PTY_ERR;
                    } else {
        ...
        if (GSM0710MUXD_SUCCESS != err)
        {
            Gsm0710Muxd_Assert(err);
        }
        if (use_ping) {
...
int main(int argc, char *argv[])
{
    ...
    if ((serial.in_buf = gsm0710_buffer_init()) == NULL ||
        (cmux_mode == 1 &&
         (serial.adv_frame_buf = (unsigned char *)calloc(1, (cmux_N1 + 3) * 2 + 2)) == NULL)) {
    ...
    while ((main_exit_signal == 0) && ((rc = watchdog(&serial)) == 0)) {
        ...
        /* Add codes for test force assert functionality by LS */
        {
            ...
            property_get("ril.mux.force.assert.flag", property_value, NULL);
            if (prop_value[0] == 'Y') {
                LOGMUX(LOG_ERR, "Detect Muxd Force Assert Flag as Y");
                property_set("ril.mux.force.assert.flag", "N");
...
#ifdef MTK_RIL_MD2
        property_set("ril.mux.report.case", "5");
#else
        property_set("ril.mux.report.case", "1");
#endif
        //property_set("ctl.start", "muxreport-daemon");
        property_set("ril.muxreport", "1");
