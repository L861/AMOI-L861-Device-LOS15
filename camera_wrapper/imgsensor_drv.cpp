...
)
{
    ...
    //init. resolution
    pSensorResInfo[0] = (ACDK_SENSOR_RESOLUTION_INFO_STRUCT*)&m_SenosrResInfo[0];
    pSensorResInfo[1] = (ACDK_SENSOR_RESOLUTION_INFO_STRUCT*)&m_SenosrResInfo[1];
...
ImgSensorDrv::setScenario(SENSOR_DRIVER_SCENARIO_T scenarioconf)
{
    ...
    for(i=0; i<1; i++) {
        if(SENSOR_SCENARIO_ID_UNNAMED_START != tempId[i]) {
            FeaturePara32 = scenarioconf.InitFPS;
            FeatureParaLen = sizeof(MUINTPTR);
            ret = featureControl((CAMERA_DUAL_CAMERA_SENSOR_ENUM)tempDevId[i], SENSOR_FEATURE_SET_FRAMERATE,
				 (MUINT8*)&FeaturePara32,(MUINT32*)&FeatureParaLen);
            if (ret < 0) {
	    ...
	    FeatureParaLen = sizeof(MUINTPTR);
	    ret = featureControl((CAMERA_DUAL_CAMERA_SENSOR_ENUM)tempDevId[i], SENSOR_FEATURE_SET_HDR,
				 (MUINT8*)&FeaturePara32,(MUINT32*)&FeatureParaLen);
	    if (ret < 0) {
		LOG_ERR("[setScenario]: SENSOR_FEATURE_SET_FRAMERATE error\n");
		return SENSOR_UNKNOWN_ERROR;
	       ...
	       /* PDAF Type */
	       {
            ...
            sensorCtrl.InvokeCamera = (CAMERA_DUAL_CAMERA_SENSOR_ENUM)scenarioconf.sensorDevId;
            sensorCtrl.ScenarioId = (MSDK_SCENARIO_ID_ENUM)tempId[i];
            ...
            FeatureParaLen = sizeof(MUINTPTR);
            ret = featureControl((CAMERA_DUAL_CAMERA_SENSOR_ENUM)tempDevId[i],SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ,
				 (MUINT8*)&FeaturePara32,(MUINT32*)&FeatureParaLen);
            if (ret < 0) {
               LOG_ERR("[init]:  SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ error\n");
               return SENSOR_UNKNOWN_ERROR;
            ...
            FeatureParaLen = sizeof(pFeaturePara16);
            ret = featureControl((CAMERA_DUAL_CAMERA_SENSOR_ENUM)tempDevId[i], SENSOR_FEATURE_GET_PERIOD,
				 (MUINT8*)pFeaturePara16,(MUINT32*)&FeatureParaLen);
            if (ret < 0) {
            ...
            if (FeaturePara32) {
                if(FeaturePara32 >= 1000) {
                    if(SENSOR_MAIN == scenarioconf.sensorDevId) {
                        m_LineTimeInus[0] = (MUINT32)(((MUINT64)pFeaturePara16[0]*1000000 +
					((FeaturePara32/1000)-1))/(FeaturePara32/1000));   // 1000 base , 33657 mean 33.657 us
                    }
                    else{
                        m_LineTimeInus[1] = (MUINT32)(((MUINT64)pFeaturePara16[0]*1000000 +
					((FeaturePara32/1000)-1))/(FeaturePara32/1000));   // 1000 base , 33657 mean 33.657 us
                    }
                }
                else {
...
)
{
    ...
    for(i=0; i<2; i++) {
	LOG_MSG("ScenarioId[%d]=%d, pSensorInfo[%d]=%p, pSensorConfigData[%d]=%p\n",
		i,ScenarioId[i],i,pSensorInfo[i],i,pSensorConfigData[i]);
...
)
{
    ...
    {
        pSensorInfo->iHDRSupport			= TRUE;
    }
    pSensorInfo->PDAF_Support                           = Info2.PDAF_Support;
    pSensorInfo->virtualChannelSupport			= FALSE;
...
{
    LOG_MSG("[%s] FeatureId=%d, pFeatureParaLen=%d \n",__FUNCTION__,FeatureId,*pFeatureParaLen);
